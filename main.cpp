...
int test() {
	...
	for (int k = 0; k < sizeof(bnch_cases) / sizeof(bnch_cases[0]); k++) {
		...
		for (auto out_order : out_orders) {
			...
			nd4j_printf("out_order %c batch_1 %d batch_2 %d M %d K %d N %d:::: ", out_order, bnch_cases[k][0], bnch_cases[k][1], bnch_cases[k][2], bnch_cases[k][3], bnch_cases[k][4]);
			time_it<outter_loops, inner_loops>(batchedGemm3<float,float,float>, total_FlOps, &a, &b, &c_actual, alpha, beta, out_order);
			...
			batchedGemm3<float, float, float>(&a, &b, &c_actual, alpha, beta, out_order);
#if 0
